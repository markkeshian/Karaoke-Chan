...
void main() {
  ...
  group('SongRepository', () {
    test('insert returns song with assigned id', () async {
      final song = await repo.insert(_song());
    ...
    test('getAll returns all inserted songs', () async {
      await repo.insert(_song(title: 'A', filePath: '/a.mp4'));
      await repo.insert(_song(title: 'B', filePath: '/b.mp4'));
    ...
    test('getAll returns songs sorted by title ASC', () async {
      await repo.insert(_song(title: 'Zebra', filePath: '/z.mp4'));
      await repo.insert(_song(title: 'Apple', filePath: '/a.mp4'));
    ...
    test('getAll filters by title search', () async {
      await repo.insert(_song(title: 'Rock Anthem', filePath: '/rock.mp4'));
      await repo.insert(_song(title: 'Pop Hits', filePath: '/pop.mp4'));
    ...
    test('getAll filters by artist search', () async {
      await repo.insert(_song(title: 'Song', artist: 'Queen', filePath: '/q.mp4'));
      await repo.insert(_song(title: 'Another', artist: 'Adele', filePath: '/a.mp4'));
    ...
    test('getAll is case-insensitive in search', () async {
      await repo.insert(_song(title: 'Summer Hits', filePath: '/s.mp4'));
    ...
    test('getById returns correct song', () async {
      final inserted = await repo.insert(_song());
    ...
    test('deleteByPath removes by file path', () async {
      await repo.insert(_song(filePath: '/target.mp4'));
    ...
    test('deleteByFolderRoot removes all matching paths', () async {
      await repo.insert(_song(title: 'A', filePath: '/music/pop/a.mp4'));
      await repo.insert(_song(title: 'B', filePath: '/music/rock/b.mp4'));
      await repo.insert(_song(title: 'C', filePath: '/other/c.mp4'));
    ...
    test('getTopPlayed returns songs ordered by play count', () async {
      final a = await repo.insert(_song(title: 'Rarely Played', filePath: '/rare.mp4'));
      final b = await repo.insert(_song(title: 'Often Played', filePath: '/often.mp4'));
    ...
    test('insert with same file_path replaces existing song', () async {
      await repo.insert(_song(title: 'Original'));
      await repo.insert(_song(title: 'Replacement')); // same filePath
    ...
    test('update changes song fields', () async {
      final song = await repo.insert(_song(title: 'Old Title'));
